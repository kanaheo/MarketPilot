...
Manual paper orders can be submitted and listed per owned portfolio. The
first order phase stores symbol, buy or sell side, market or limit type,
quantity, portfolio currency, strategy version, and decision evidence.
Accepted orders remain `PENDING`; no cash or holdings are changed until the
separate execution phase has a trustworthy market price and fill rule.
...
소유한 포트폴리오별로 수동 모의주문을 접수하고 목록을 조회할 수 있습니다. 첫 주문
단계에서는 종목 코드, 매수·매도, 시장가·지정가, 수량, 포트폴리오 통화, 전략 버전,
판단 근거를 저장합니다. 접수된 주문은 `PENDING` 상태이며, 신뢰할 수 있는 시장
가격과 체결 규칙을 만드는 별도 체결 단계 전까지 현금과 보유 종목을 변경하지
않습니다.
...
所有するポートフォリオごとに手動ペーパー注文を登録し、一覧を取得できます。
最初の注文フェーズでは、銘柄コード、売買区分、成行・指値、数量、ポートフォリオ
通貨、戦略バージョン、判断根拠を保存します。登録された注文は`PENDING`状態で、
信頼できる市場価格と約定ルールを用意する別の約定フェーズまでは、現金と保有銘柄を
変更しません。
