...
match the value in `apps/api/.env`. Generate a separate
`MARKETPILOT_USER_API_SIGNING_SECRET` for short-lived authenticated user API
tokens and copy it to both applications. The two MarketPilot secrets must be
different and must never use a `NEXT_PUBLIC_` prefix. Do not commit
`.env.local`.
...
`apps/api/.env`와 정확히 같아야 합니다. 로그인 사용자 API의 짧은 수명 토큰에는
별도의 `MARKETPILOT_USER_API_SIGNING_SECRET`을 생성해 두 애플리케이션에 같은
값으로 입력합니다. 두 MarketPilot 비밀값은 서로 달라야 하며 `NEXT_PUBLIC_`
접두사를 붙이면 안 됩니다. `.env.local`은 커밋하지 않습니다.
...
`apps/api/.env`と完全に一致させます。認証済みユーザーAPIの短命トークン用に
別の`MARKETPILOT_USER_API_SIGNING_SECRET`を生成し、両方のアプリへ同じ値を
設定します。2つのMarketPilotシークレットは別の値にし、`NEXT_PUBLIC_`を
付けないでください。`.env.local`はコミットしません。
