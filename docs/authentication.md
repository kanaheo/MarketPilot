...
Email authentication and account linking are also future work.
...
- Permanent users and linked accounts are not part of this implementation
  step.
...
- 영구 사용자 저장과 계정 연결은 이번 구현 범위에 포함되지 않습니다.
...
- 永続ユーザー保存とアカウント連携は今回の実装範囲に含まれません。
